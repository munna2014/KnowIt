  ...
  const handleSubmit = useCallback(async (event, status = form.status) => {
    ...
    try {
      ...
      if (err.status === 401) {
        ...
        setError(err?.message || "Could not create blog post. Please try again.");
      }
    } finally {
      setIsSubmitting(false);
    }
  }, [form, featuredImage, router]);
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="rounded-xl border border-slate-700/60 bg-slate-900/70 p-6 shadow-xl">
              ...
              {(featuredImagePreview || form.featuredImageUrl) && (
                <div className="mb-4">
                  <img
                    src={featuredImagePreview || form.featuredImageUrl}
            ...
            <div className="flex justify-between items-center gap-4 pt-6">
              <button
                type="button"
                onClick={() => router.push("/component/landing")}
